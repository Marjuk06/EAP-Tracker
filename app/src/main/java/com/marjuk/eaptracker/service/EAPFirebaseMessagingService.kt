        ...
        fun uploadFcmToken(context: Context, token: String) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    // Also push via REST with secret key
                    val urlStr = "$RTDB_URL/students/$studentKey/fcm_token.json?auth=yleDrwUbDfga8ueOKchQEZ47XCPlrzlRNiCF1NAw"
                    ...
                    conn.outputStream.use { os ->
                        os.write("\"$token\"".toByteArray(Charsets.UTF_8))
                    }
                    val code = conn.responseCode
                    conn.disconnect()
                    Log.d(TAG, "Uploaded student FCM token to RTDB for $studentKey (HTTP $code)")
                } catch (e: Exception) {
