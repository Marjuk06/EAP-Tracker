...
class ScheduleSyncWorker(
    ...
    private fun fetchAndDeliverBroadcastNotification() {
        try {
            val urlStr = "$BASE_URL/broadcast_notification.json"
            val url = URL(urlStr)
    ...
    private fun fetchAndDeliverMasterEapData() {
        try {
            val urlStr = "$BASE_URL/eap_data.json"
            val url = URL(urlStr)
            val conn = url.openConnection() as HttpURLConnection
            conn.requestMethod = "GET"
    ...
    private fun fetchAndDeliverPersonalNotice(studentKey: String) {
        try {
            val urlStr = "$BASE_URL/students/$studentKey/personal_notice.json"
            val url = URL(urlStr)
            val conn = url.openConnection() as HttpURLConnection
            conn.requestMethod = "GET"
            conn.connectTimeout = 10000
    ...
    private fun fetchAndDeliverGlobalNotification() {
        try {
            val urlStr = "$BASE_URL/global_notifications/latest.json"
            val url = URL(urlStr)
            val conn = url.openConnection() as HttpURLConnection
            conn.requestMethod = "GET"
            conn.connectTimeout = 10000
    ...
    private fun fetchAndDeliverTargetExamAndQuote() {
        try {
            ...
            val examUrlStr = "$BASE_URL/target_exam_config.json"
        ...
        try {
            ...
            val quoteUrlStr = "$BASE_URL/daily_quote_config.json"
